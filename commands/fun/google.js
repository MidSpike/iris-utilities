...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts={}) {
        ...
        try {
            const google_search_results = await googleIt({'options':{'no-display':true}, 'query':google_search_query})
            bot_message.edit(new CustomRichEmbed({
                title:`Searched Google For:`,
                description:`${'```'}\n${google_search_query}\n${'```'}`,
                fields:google_search_results.map(result => ({name:`${result.title}`, value:`<${result.link}>\n${result.snippet}`}))
            }, message));
        } catch (error) {
            console.trace(error);
        }
