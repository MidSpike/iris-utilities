...
const { Discord, client } = require('./src/libs/bot.js');

const { Timer, getReadableTime } = require('./src/utilities.js');
...
const { isThisBot, isThisBotsOwner, isSuperPerson, isSuperPersonAllowed } = require('./src/libs/permissions.js');
...
} = require('./src/libs/DisBotCommander.js');
...
async function updateGuildConfig(guild) {
    ...
    const new_guild_config = {
        ...
        ...{
            ...
            _has_permissions: `${
                guild.me.hasPermission('ADMINISTRATOR') ? 'ADMINISTRATOR' : guild.me.permissions.toArray()
            }`,
...
function checkForBlacklistedGuild(guild) {
    ...
    const guild_is_blacklisted = blacklisted_guilds
        .map((blacklisted_guild) => blacklisted_guild.id)
        .includes(guild?.id);
...
async function checkForBlacklistedUser(message) {
    ...
    if (blacklisted_users.map((blacklisted_user) => blacklisted_user.id).includes(message.author.id)) {
        ...
        const bot_support_guild_invite = await generateInviteToGuild(
            bot_support_guild_id,
            'Generated by: checkForBlacklistedUser',
        ).catch(console.trace);
...
client.on('ready', async () => {
    ...
    console.log(
        `${bot_common_name} Logged in as ${client.user.tag} on ${ready_timestamp} in ${client.guilds.cache.size} servers!`,
    );
    console.log(
        `----------------------------------------------------------------------------------------------------------------`,
    );
    ...
    client.setTimeout(() => {
        const guild_restart_logging_channels = client.channels.cache.filter(
            (channel) => channel.type === 'text' && channel.name === bot_restart_log_channel.name,
        );
        guild_restart_logging_channels.forEach((channel) => {
            if (channel.permissionsFor(channel.guild.me).has('SEND_MESSAGES')) {
                ...
                console.warn(
                    `Unable to send restart message to ${channel.name} (${channel.id}) > ${channel.guild.name} (${channel.guild.id})`,
                );
            }
        });
    }, 1000 * 60 * 1); // 1 minute
    ...
    for (const guild_id of all_guild_configs.keys()) {
        const guild_exists_to_the_bot = await client.guilds
            .fetch(guild_id)
            .then(() => true)
            .catch(() => false);
        if (guild_exists_to_the_bot) continue; // the guild exists to the bot, so continue through the list
        console.warn(
            `Guild (${guild_id}) from the guild configs, is not accessible by the bot; it most likely removed the bot!`,
        );
...
client.on('guildCreate', async (guild) => {
    ...
    if (central_guild_history_logging_channel) {
        central_guild_history_logging_channel
            .send(
                new CustomRichEmbed({
                    color: 0x00ff00,
    ...
    const bot_support_guild_invite = await generateInviteToGuild(
        bot_support_guild_id,
        'Generated by: guildCreate',
    ).catch(console.trace);
    const viewable_text_channels = guild.channels.cache.filter(
        (c) => c.type === 'text' && c.viewable && c.permissionsFor(guild.me).has('SEND_MESSAGES'),
    );
    const potential_bot_commands_channel = viewable_text_channels
        .filter((c) => ['bot-commands', 'commands', 'bot'].includes(c.name))
        .first();
    const potential_general_channel = viewable_text_channels
        .filter((c) => ['general-chat', 'general', 'chat'].includes(c.name))
        .first();
    const fallback_first_available_channel = viewable_text_channels.first();
    const channel_to_send_initial_message =
        potential_general_channel ?? potential_bot_commands_channel ?? fallback_first_available_channel ?? undefined;
                ...
                channel
                    .send(
                        new CustomRichEmbed({
                            ...
                            description:
                                'I was unable to modify the permissions of this channel to only allow myself to send messages in it!',
                        }),
                    )
                    .catch(console.warn);
            });
            ...
            channel
                .send(
                    new CustomRichEmbed({
                        ...
                        title: 'Warning!',
                        description:
                            "If I don't have the `MANAGE_GUILD` and `VIEW_AUDIT_LOG` permissions, I will need them to see invite events for all channels!",
                    }),
                )
                .catch(console.warn);
            break;
...
client.on('guildMemberAdd', async (member) => {
    ...
    const logging_channel = member.guild.channels.cache.find(
        (channel) => channel.name === bot_members_log_channel.name,
    );
    ...
    logging_channel
        ...
        .catch(() => {
            console.warn(
                `Unable to send message to ${logging_channel.guild.name} (${logging_channel.guild.id}) > ${logging_channel.name} (${logging_channel.id})`,
            );
        });
});
...
client.on('messageReactionAdd', async (reaction, user) => {
    ...
    if (reaction.message.partial)
        await reaction.message.fetch().catch((warning) => console.warn('1599589897074415111', warning));
    ...
    const logging_channel = reaction.message.guild.channels.cache.find(
        (channel) => channel.name === bot_reaction_log_channel.name,
    );
    if (!logging_channel) return;
    logging_channel
        .send(
...
client.on('messageReactionRemove', async (reaction, user) => {
    ...
    if (reaction.message.partial)
        await reaction.message.fetch().catch((warning) => console.warn('1599589897074481368', warning));
...
client.on('inviteCreate', async (invite) => {
    ...
    const logging_channel = invite.channel.guild.channels.cache.find(
        (channel) => channel.name === bot_invite_log_channel.name,
    );
    if (!logging_channel) return;
    ...
    logging_channel
        ...
        .catch(() => {
            console.warn(
                `Unable to send \'inviteCreate\' message to ${logging_channel.guild.name} (${logging_channel.guild.id}) > ${logging_channel.name} (${logging_channel.id})`,
            );
        });
});
...
client.on('inviteDelete', async (invite) => {
    ...
    const guild_audit_logs = await invite.channel.guild
        .fetchAuditLogs({
            limit: 1,
            type: 'INVITE_DELETE',
        })
    ...
    logging_channel
        ...
        .catch(() => {
            console.warn(
                `Unable to send 'inviteDelete' message to ${logging_channel.guild.name} (${logging_channel.guild.id}) > ${logging_channel.name} (${logging_channel.id})`,
            );
        });
});
...
client.on('guildMemberAdd', async (member) => {
    ...
    for (const guild of client.guilds.cache.values()) {
        ...
        if (member_is_banned_in_guild) {
            // the GuildMember is not banned in this server with the bot
            ...
            const potential_purgatory_channel = bot_appeals_guild.channels.cache.find(
                (channel) => channel.name === `${guild_with_banned_member.id}-${banned_guild_member.id}`,
            );
            const bot_purgatory_channel =
                potential_purgatory_channel ??
                (await bot_appeals_guild.channels.create(`${guild_with_banned_member.id}-${banned_guild_member.id}`, {
                    type: 'text',
            ...
            const message_collector = bot_purgatory_channel.createMessageCollector(collection_filter, {
                max: 1,
                maxProcessed: 1,
            });
            message_collector.on('collect', async (collected_message) => {
                await bot_purgatory_channel
                    .send(
                        new CustomRichEmbed({
                            author: {
                                iconURL: collected_message.member.user.displayAvatarURL({ dynamic: true }),
                                name: `@${collected_message.member.user.tag} (${collected_message.member.user.id})`,
                            },
                ...
                const guild_purgatory_channel = guild.channels.cache.filter(
                    (channel) => channel.type === 'text' && channel.name === bot_appeals_log_channel.name,
                );
                await guild_purgatory_channel
                    .send(
                        new CustomRichEmbed({
                            author: {
                                iconURL: collected_message.member.user.displayAvatarURL({ dynamic: true }),
...
client.on('message', async (message) => {
    ...
    if (message.user?.partial)
        await message.user.fetch().catch((warning) => console.warn('1599589897074640420', warning));
    if (message.member?.partial)
        await message.member.fetch().catch((warning) => console.warn('1599589897074997570', warning));
    ...
    if (message.channel.type === 'dm') {
        ...
        const potential_central_dm_channel_with_user = bot_logging_guild.channels.cache.find(
            (channel) => channel.name === `dm-${message.author.id}`,
        );
        if (potential_central_dm_channel_with_user) {
            ...
            await message.channel
                ...
                .catch(console.warn);
            const central_dm_channel_with_user = await bot_logging_guild.channels
                .create(`dm-${message.author.id}`, {
                    type: 'text',
                    topic: `${message.author.tag} (${message.author.id}) | ${moment()}`,
                })
                .catch(console.trace);
            await central_dm_channel_with_user
                .setParent(process.env.CENTRAL_DM_CHANNELS_CATEGORY_ID)
                .catch(console.trace);
...
client.on('message', async (message) => {
    ...
    if (message.user?.partial)
        await message.user.fetch().catch((warning) => console.warn('1599589897074181056', warning));
    if (message.member?.partial)
        await message.member.fetch().catch((warning) => console.warn('1599589897074955328', warning));
    ...
    if (typeof command_prefix !== 'string' || command_prefix.length === 0) {
        console.error(
            `Guild (${message.guild.id}) has an invalid command prefix: ${command_prefix}; manual fixing is required!`,
        );
        return;
    }
    ...
    const contains_invite_link =
        message.cleanContent.includes(`discord.gg/`) ||
        message.cleanContent.includes('discord.com/invite/') ||
        message.cleanContent.includes(`discord.io/`) ||
        message.cleanContent.includes(`invite.gg/`);
                ...
                .send(
                    new CustomRichEmbed({
                        ...
                        description: `Sending discord invites is not allowed in this guild${
                            _member_is_immune ? ', but you are immune!' : '!'
                        }`,
                    }),
                )
                .catch(console.warn);
    ...
    const contains_url = new RegExp(
        '([a-zA-Z0-9]+://)?([a-zA-Z0-9_]+:[a-zA-Z0-9_]+@)?([a-zA-Z0-9.-]+\\.[A-Za-z]{2,4})(:[0-9]+)?(/.*)?',
    ).test(message.cleanContent);
    ...
    const guild_command_prefix_is_default =
        guild_config.command_prefix === bot_config.DEFAULT_GUILD_CONFIG.command_prefix;
    if ((guild_is_a_known_bot_list || guild_is_an_unknown_bot_list) && guild_command_prefix_is_default) {
        console.error(
            `Guild [${message.guild.name}] (${message.guild.id}) should not have the default command_prefix!`,
        );
        return;
    }
    ...
    const fetched_allowed_channels = await Promise.all(
        guild_allowed_channels.map(async (channel_id) => await message.guild.channels.resolve(channel_id)?.fetch()),
    );
    ...
    if (
        guild_allowed_channels.length > 0 &&
        is_not_backup_commands_channel &&
        !is_guild_allowed_channel &&
        !member_is_immune_from_channel_exclusions
    ) {
        const dm_channel = await message.author.createDM();
        dm_channel.send(
            new CustomRichEmbed({
    ...
    try {
        ...
        const current_command_logs = command_log_file_exists
            ? JSON.parse(fs.readFileSync(current_command_log_file_name))
            : [];
    ...
    const central_anonymous_command_logging_channel = client.channels.cache.get(
        bot_central_anonymous_command_log_channel_id,
    );
    central_anonymous_command_logging_channel
        .send(`${'```'}json\n${JSON.stringify(anonymous_command_log_entry, null, 2)}\n${'```'}`)
        .catch(console.trace);

    /* guild command logging */
    const guild_command_logging_channel = message.guild.channels.cache.find(
        (channel) => channel.type === 'text' && channel.name === bot_command_log_channel.name,
    );
    if (guild_command_logging_channel) {
        guild_command_logging_channel
            .send(
                new CustomRichEmbed({
                    author: {
                        iconURL: message.author.displayAvatarURL({ dynamic: true }),
    ...
    const hasGuildModeratorRole =
        message.member.roles.cache.filter((role) => guild_config.moderator_roles?.includes(role.id)).size > 0;
    const hasGuildAdminRole =
        message.member.roles.cache.filter((role) => guild_config.admin_roles?.includes(role.id)).size > 0;
    ...
    if (command_author_access_level < command.access_level) {
        ...
        if (
            [DisBotCommander.categories.ADMINISTRATOR, DisBotCommander.categories.GUILD_SETTINGS].includes(
                command.category,
            )
        ) {
