    ...
    if (!message.guild.me.permissions.has(required_permissions)) {
        message.channel.send(new CustomRichEmbed({
            ...
            description: [
                ...
                `You cannot perform this command when I don\'t have the required permission(s)!`,
            ].join(''),
        }, message));
        return false;
    } else {
...
const isSuperPersonAllowed = (super_person, permission_flag) => {
    if (super_person) {
        if (isThisBotsOwner(super_person.id)) {
            ...
            const has_permission = !denied_all_permissions && !denied_permission_flag && (allowed_all_permissions || allowed_permission_flag);
