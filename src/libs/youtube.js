...
async function forceYouTubeSearch(search_query, max_results = 5, retry_attempts = 1) {
    ...
    if (Math.floor(max_results) !== max_results || max_results < 1)
        throw RangeError('`max_results` must be a whole number and at least `1`!');
    if (isNaN(retry_attempts)) throw new TypeError('`retry_attempts` must be positive whole number above zero!');
    if (Math.floor(retry_attempts) !== retry_attempts || retry_attempts < 1)
        throw RangeError('`retry_attempts` must be a whole number and at least `1`!');
...
async function playYouTube(message, search_query, playnext = false) {
    ...
    async function _get_video_id_from_query(query) {
        ...
        if (validator.isURL(query ?? '')) {
            ...
            // search for the video via the youtube api as a fallback
    ...
    async function _play_as_video(video_id, send_embed = true) {
        ...
        const youtube_playlist_api_response = await axios.get(
            `${bot_api_url}/ytinfo?token=${encodeURIComponent(process.env.BOT_API_SERVER_TOKEN)}&video_id=${encodeURI(
                video_id,
            )}`,
        );
        ...
        await guild_queue_manager.addItem(
            new QueueItem('youtube', player, `${yt_video_info.videoDetails.title}`, { videoInfo: yt_video_info }),
    ...
    if (potential_playlist_id) {
        // the search_query was a playlist
        try {
        ...
        // the search_query is a video
        await _play_as_video(potential_video_id);
    } else {
        // the search_query is unknown
        if (!search_message.deleted) await search_message.delete({ timeout: 500 }).catch(console.warn);
        message.channel.send(
            new CustomRichEmbed(
                {
