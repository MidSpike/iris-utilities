...
const { Timer, isObject, array_insert, array_shuffle, random_range_inclusive } = require('../utilities.js');
...
class QueueItem {
    ...
    constructor(type, player, description, metadata = {}) {
        if (!QueueItem.types.includes(type))
            throw new TypeError('`type` should be a valid type from `QueueItem.types`');
        if (typeof player !== 'function')
            throw new TypeError('`player` should be an anonymous function created by `QueueItemPlayer`');
...
class QueueItemPlayer {
    ...
    constructor(
        ...
        return async () => {
            ...
            const guild_tts_voice =
                guild_tts_provider === 'ibm' ? guild_config.tts_voice_ibm : guild_config.tts_voice_google;

            const guild_audio_controller = guild.client.$.audio_controllers.get(guild.id);

            playStream(
                await createConnection(this.voice_connection.channel),
                await this.stream_maker(),
                this.volume_ratio,
                async () => {
                ...
                async (voice_connection, dispatcher) => {
                    ...
                    const queue_is_active = () =>
                        this.queue_manager.queue.length > 0 ||
                        this.queue_manager.loop_enabled ||
                        this.queue_manager.autoplay_enabled;
                    if (!queue_is_active()) {
                        // the bot is not active in vc, so run the disconnect process
                        ...
                        if (guild_config.disconnect_tts_voice === 'enabled') {
                            ...
                            const tts_url_stream = `${process.env.BOT_API_SERVER_URL}/speech?token=${encodeURIComponent(
                                process.env.BOT_API_SERVER_TOKEN,
                            )}&type=${encodeURIComponent(guild_tts_provider)}&lang=${encodeURIComponent(
                                guild_tts_voice,
                            )}&text=${encodeURIComponent(tts_text)}`;
                            playStream(voice_connection, tts_url_stream, 15.0, undefined, () => {
                                guild_audio_controller.disconnect();
                            });
                        } else {
                            // disconnect without TTS
                            guild_audio_controller.disconnect();
                        }
                    } else {
                        // the queue is not empty so handle it
                        if (this.queue_manager.loop_enabled) {
                            ...
                            if (this.queue_manager.loop_type === 'multiple') {
                                ...
                                const random_queue_insertion = random_range_inclusive(
                                    1,
                                    this.queue_manager.queue.length,
                                );
                        ...
                        if (this.queue_manager.queue.length > 0) {
                            // the queue is not empty so continue playing
                            if (guild_config.queue_tts_voice === 'enabled') {
                                const tts_text = `Next Up: ${this.queue_manager.queue[0].description}`;
                                const tts_url_stream = `${
                                    process.env.BOT_API_SERVER_URL
                                }/speech?token=${encodeURIComponent(
                                    process.env.BOT_API_SERVER_TOKEN,
                                )}&type=${encodeURIComponent(guild_tts_provider)}&lang=${encodeURIComponent(
                                    guild_tts_voice,
                                )}&text=${encodeURIComponent(tts_text)}`;
