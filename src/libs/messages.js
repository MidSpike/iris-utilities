...
async function sendLargeMessage(channel_id, large_message, code_block_lang = '') {
    ...
    for (const message_chunk of message_chunks) {
        let sent_message;
        try {
            sent_message = await client.channels.cache
                .get(channel_id)
                .send(`${'```'}${code_block_lang}\n${message_chunk}\n${'```'}`);
        } catch (error) {
            console.warn(error);
        } finally {
...
async function sendOptionsMessage(
    channel_id,
    embed,
    reaction_options = options_message_reactions_template,
    ...
    for (const reaction_option of reaction_options) {
        // execute each reaction sequentially
        if (options_message.deleted) break; // don't add reactions to deleted messages

        const reaction_option_emoji =
            findCustomEmoji(reaction_option.emoji_name) ?? nodeEmoji.emojify(nodeEmoji.get(reaction_option.emoji_name));
 ...
 * @deprecated
 */
function sendConfirmationEmbed(
    confirm_user_id,
    channel_id,
    delete_after_selection = true,
    embed_contents = 'Default Embed',
    ...
    sendOptionsMessage(channel_id, makeEmbed(), [
        {
            ...
            callback(options_message, collected_reaction, user) {
                ...
                if (audio_controller.paused === true) {
                    ...
                    // There isn't an active connection with music
                    options_message.edit(
                        new CustomRichEmbed({
                            color: 0xffff00,
                            author: { iconURL: user.displayAvatarURL({ dynamic: true }), name: `@${user.tag}` },
            ...
            callback(options_message, collected_reaction, user) {
                ...
                options_message.edit(
                    new CustomRichEmbed({
                        ...
                        title: `${embed_title}`,
                        description: `${
                            guild_queue_manager.loop_enabled ? 'Started' : 'Stopped'
            ...
            callback(options_message, collected_reaction, user) {
                ...
                options_message.edit(
                    new CustomRichEmbed({
                        ...
                        description: `${
                            guild_queue_manager.loop_enabled ? 'Started' : 'Stopped'
                        } Looping The First Item In The Queue`,
                    }),
                );
            },
        },
...
async function sendYtDiscordEmbed(user_message, videoInfo, status = 'Playing') {
    ...
    function makeYTEmbed() {
        const status_override =
            guild_queue_manager.queue.length <= 1
                ? guild_queue_manager.loop_enabled
                    ? 'Looping'
                    : guild_queue_manager.autoplay_enabled
                    ? 'Autoplaying'
                    : undefined
                : undefined;
        ...
        return new CustomRichEmbed(
            {
                ...
                description: show_player_description
                    ? [
                          ...
                          `Rating: ${((videoInfo.videoDetails.averageRating / 5) * 100).toFixed(
                              2,
                          )}% of people like this`,
                ...
                thumbnail: show_player_description
                    ? `${bot_cdn_url}/youtube_logo.png`
                    : `${videoInfo.videoDetails.thumbnail.thumbnails.slice(-1).pop().url}`,
                image: show_player_description
                    ? `${videoInfo.videoDetails.thumbnail.thumbnails.slice(-1).pop().url}`
                    : undefined,
            },
            user_message,
    ...
    const embed = new CustomRichEmbed(
        {
            ...
            }${'```'}Try contacting a guild admin or an ${
                bot_config.COMMON_NAME
            } admin if you believe this to be in fault.`,
            footer: null,
        },
        message,
    );
...
function logAdminCommandsToGuild(admin_message, custom_log_message = undefined) {
    const moderation_log_channel = admin_message.guild.channels.cache.find(
        (c) => c.name === bot_config.SPECIAL_CHANNELS.find((ch) => ch.id === 'GUILD_MODERATION').name,
    );
