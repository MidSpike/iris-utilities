...
function detect_unsupported_urls(search_query) {
    if (
        search_query.includes('spotify.com/') ||
...
async function playRemoteMP3(message, remote_mp3_path, playnext = false) {
    ...
    guild_queue_manager
        .addItem(new QueueItem('mp3', player, `MP3`, { mp3_file_name: `${remote_mp3_path}` }), playnext ? 2 : undefined)
        .then(() => {
...
async function playUserUploadedMP3(message, playnext = false) {
    ...
    if (message_media) {
        if (message_media.attachment.endsWith('.mp3')) {
            ...
            guild_queue_manager
                .addItem(
                    new QueueItem('mp3', player, `MP3`, { mp3_file_name: `${message_media.name}` }),
...
async function playBroadcastify(message, search_query, playnext = false) {
    ...
    guild_queue_manager
        .addItem(new QueueItem('other', player, `Broadcastify Stream`), playnext ? 2 : undefined)
        .then(() => {
            if (guild_queue_manager.queue.length > 1) {
                message.channel.send(
                    new CustomRichEmbed(
