        ...
        if (guild_queue_manager.queue.length > 0) {
                ...
                if (['items', 'i'].includes(command_args[0])) {
                    ...
                    function makeQueueEmbed() {
                        const entire_queue_formatted = guild_queue_manager.queue.map((queue_item, index) => {
                            if (queue_item.type === 'youtube') {
                                const yt_video_title = `${string_ellipses(
                                    queue_item.metadata.videoInfo.videoDetails.title,
                                    50,
                                )}`;
                                const yt_video_link = `https://youtu.be/${queue_item.metadata.videoInfo.videoDetails.videoId}`;
                                const yt_channel_title = `${string_ellipses(
                                    queue_item.metadata.videoInfo.videoDetails.author.name,
                                    50,
                                )}`;
                            ...
                            return new CustomRichEmbed(
                                {
                                    ...
                                    description: `There are currently ${
                                        guild_queue_manager.queue.length
                                    } items in the queue.\nCurrently on queue page: \`[ ${page_index + 1} ]\` of \`[ ${
                                        queue_pages.length
                                    } ]\`.`,
                    ...
                    message.channel.send(
                        new CustomRichEmbed(
                            {
                                title: `${
                                    guild_queue_manager.autoplay_enabled ? 'Enabled' : 'Disabled'
                                } Autoplay Of Related YouTube Videos In The Queue`,
                            },
                            message,
                        ),
                    );
                    ...
                    if (['item', 'i'].includes(command_args[1])) {
                        ...
                        message.channel.send(
                            new CustomRichEmbed(
                                {
                                    title: `${
                                        guild_queue_manager.loop_enabled ? 'Enabled' : 'Disabled'
                                    } Queue Looping For The First Item`,
                                },
                                message,
                            ),
                        );
                            ...
                            new CustomRichEmbed(
                                {
                                    title: `${
                                        guild_queue_manager.loop_enabled ? 'Enabled' : 'Disabled'
                                    } Queue Looping For The Entire Queue`,
                                },
                                message,
                            ),
                        );
                            ...
                            new CustomRichEmbed(
                                {
                                    title: `${
                                        guild_queue_manager.loop_enabled ? 'Enabled' : 'Disabled'
                                    } Queue Shuffle Looping For The Entire Queue`,
                                },
                                message,
                            ),
                        );
                            ...
                            new CustomRichEmbed(
                                {
                                    ...
                                    description: `${'```'}\n${['i | item', 'a | all', 's | shuffle']
                                        .map((item) => `${discord_command} ${command_args[0]} [ ${item} ]`)
                                        .join('\n')}${'```'}`,
                                },
                                message,
                            ),
                                ...
                                new CustomRichEmbed(
                                    {
                                        color: 0xffff00,
                                        title: `Woah dude!`,
                                        description: `I can't remove ${remove_index_number} from the queue!\nTry specifying a number!`,
                                    },
                                    message,
                                ),
                            );
                            ...
                            new CustomRichEmbed(
                                {
                                    ...
                                    description: `Simply specify the index of the song!\nExample Usage:${'```'}\n${discord_command} ${
                                        command_args[0]
                                    } 2${'```'}The above will remove the 2nd item in the queue.`,
                                },
                                message,
                            ),
                        );
            ...
            message.channel.send(
                new CustomRichEmbed(
                    {
                        color: 0xffff00,
                        title: `Does Not Compute! The Queue Is Empty!`,
