...
const { forceYouTubeSearch, playYouTube } = require('../../libs/youtube.js');
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        if (search_query.length > 0) {
            ...
            const reactions = search_results.map((search_result, index) => ({
                emoji_name: `bot_emoji_${
                    ['zero', 'one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'][index + 1]
                }`,
            ...
            client.setTimeout(() => {
                // Wait 2 minutes before removing the search menu
