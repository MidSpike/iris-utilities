...
const { isThisBot, isThisBotsOwner, isSuperPerson, botHasPermissionsInGuild } = require('../../libs/permissions.js');
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        if (discord_command === `${command_prefix}unban`) {
            message.guild.members
                .unban(user_to_modify, `@${message.author.tag} used the ${discord_command} command!`)
                .then((unbanned_user) => {
            ...
            // assuming: discord_command === `${command_prefix}ban`
            function staffMemberCanBanUser(staff_id, user_id) {
                ...
                const staff_member_can_ban_member =
                    staff_member.roles.highest.comparePositionTo(member_being_banned.roles.highest) > 0;
            ...
            sendConfirmationEmbed(
                ...
                async () => {
                    ...
                    if (guild_member_to_ban?.bannable) {
                        ...
                        const appeals_guild_invite = await generateInviteToGuild(
                            bot_appeals_guild_id,
                            `Generated using ${discord_command} in ${message.guild.name} (${message.guild.id})`,
                        );
