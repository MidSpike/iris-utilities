...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts = {}) {
                        ...
                        fields: [
                            {
                                ...
                                value:
                                    'This command can be used to "yeet" (move) users from your voice channel into the afk channel, falling back to a random voice channel!',
                            },
                            {
                                name: 'Example Usages',
                                value: [
                                    `Yeeting mentioned members:${'```'}\n${discord_command} @user1 @user2 @user3\n${'```'}`,
                                    `Yeeting all people from the voice channel:${'```'}\n${discord_command} all\n${'```'}`,
                                ].join(''),
                            },
                        ],
                    },
        ...
        for (const member_to_yeet of members_to_yeet.values()) {
            ...
            const available_vcs = message.guild.channels.cache.filter(
                (c) =>
                    c.type === 'voice' &&
                    c.id !== message.member.voice.channel.id &&
                    c.permissionsFor(message.guild.me).has('MOVE_MEMBERS'),
            );
