...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        if ([`clear`].includes(command_args[0])) {
            ...
            const new_user_warnings = user_to_remove_warnings_from
                ? all_users_warnings.filter((warning) => warning.user_id !== user_to_remove_warnings_from.id)
                : [];
                ...
                new CustomRichEmbed(
                    {
                        title: `Removed all warnings ${
                            user_to_remove_warnings_from ? `for @${user_to_remove_warnings_from.tag} ` : ''
        ...
        const user_warnings = user_to_search_for
            ? all_users_warnings.filter((user_warning) => user_warning.user_id === user_to_search_for.id)
            : all_users_warnings;
        ...
        function makeEmbed() {
            return new CustomRichEmbed(
                {
                    title: `Here are the warnings for ${
                        user_to_search_for ? `@${user_to_search_for.user.tag}` : 'all users'
                    }!`,
                    description: [
