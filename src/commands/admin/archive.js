...
const bot_archived_channels_category_name = bot_special_channels.find((ch) => ch.id === 'ARCHIVED_CHANNELS_CATEGORY')
    .name;
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        sendConfirmationEmbed(
            ...
            async (bot_message) => {
                ...
                const potential_category_channel = message.guild.channels.cache.find(
                    (channel) => channel.type === 'category' && channel.name === bot_archived_channels_category_name,
                );
                const category_to_make =
                    potential_category_channel ??
                    (await message.guild.channels.create(`${bot_archived_channels_category_name}`, {
                        type: 'category',
