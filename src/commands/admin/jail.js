...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts = {}) {
                ...
                new CustomRichEmbed(
                    {
                        ...
                        description:
                            'This command can prevent a specified member from typing or speaking in any channel!',
                        fields: [
                            {
        ...
        function staffMemberCanJailMember(staff_id, member_id) {
            ...
            const staff_member_has_permissions = staff_member.hasPermission(
                'MANAGE_CHANNELS',
                'MANAGE_MESSAGES',
                'MUTE_MEMBERS',
            );
            ...
            const staff_member_can_jail_member =
                staff_member.roles.highest.comparePositionTo(member_being_jailed.roles.highest) > 0;
        ...
        if (discord_command === `${command_prefix}jail`) {
            ...
            // assuming: discord_command === `${command_prefix}unjail`
            const bot_message = await message.channel.send(
                new CustomRichEmbed(
                    {
