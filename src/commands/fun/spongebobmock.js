    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        if (user_text.length === 0) {
                ...
                new CustomRichEmbed(
                    {
                        ...
                        fields: [{ name: 'Example Usage', value: `${discord_command} Spongebob is going to mock me!` }],
                    },
                    message,
                ),
            );
            ...
            const api_response = await axios.get(
                `${bot_api_url}/spmock?token=${encodeURIComponent(
                    process.env.BOT_API_SERVER_TOKEN,
                )}&text=${encodeURIComponent(user_text)}`,
            );

            const { original_text, spmock_text } = api_response.data;
