...
const {
    ...
    sendOptionsMessage,
} = require('../../libs/messages.js');
const { zero_to_nine_as_words, findCustomEmoji, constructNumberUsingEmoji } = require('../../libs/emoji.js');
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        async function _send_questions() {
            async function _proceed_with_game(rich_embed, step_num) {
                ...
                if (akinator_api.progress >= 70 || akinator_api.currentStep >= 78) {
                    // Akinator has a guess for us!
                        ...
                        new CustomRichEmbed(
                            {
                                ...
                                fields: [
                                    ...
                                    { name: 'Character Description', value: `${akinator_guess.description}` },
                                    {
                                        ...
                                        value: `[Link](${akinator_guess.absolute_picture_path})`,
                                    },
            ...
            const options_embed = new CustomRichEmbed(
                {
                    ...
                    fields: [
                        ...
                        {
                            name: 'Answers',
                            value: `${akinator_api.answers
                                .map((value, index) => `${constructNumberUsingEmoji(index + 1)} - ${value}`)
                                .join('\n')}`,
                        },
                    ],
                },
