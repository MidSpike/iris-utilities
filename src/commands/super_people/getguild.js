    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        switch (`${command_args[0]}`.toLowerCase()) {
            case 'roles':
                ...
                sendLargeMessage(
                    message.channel.id,
                    sorted_guild_roles.map((role) => `(${role.id}) (${role.position}) ${role.name}`).join('\n'),
                );
                break;
            case 'invites':
                guild
                    ...
                    .then((invites) => {
                        if (invites.size && invites.size > 0) {
                            ...
                            sendLargeMessage(
                                message.channel.id,
                                invites
                                    .map((invite) => `(${invite.code}) https://discord.gg/${invite.code}`)
                                    .join('\n'),
                            );
                        } else {
            ...
            case 'channels':
                ...
                sendLargeMessage(
                    ...
                    guild.channels.cache
                        .map((channel) => `(${channel.id}) [${channel.type}] ${channel.name}`)
                        .join('\n'),
                );
                break;
            case 'managers':
                message.channel.send(`Here are the guild managers for: ${guild.id}`);
                const guild_managers = guild.members.cache.filter(
                    (m) => !m.user.bot && m.hasPermission(['MANAGE_GUILD']),
                );
                const sorted_guild_managers = guild_managers.sort(
                    (a, b) => b.roles.highest.position - a.roles.highest.position,
                );
                sendLargeMessage(
                    message.channel.id,
                    sorted_guild_managers.map((member) => `(${member.id}) ${member.user.tag}`).join('\n'),
                );
                break;
            case 'members':
                ...
                function _output_members() {
                    ...
                    const sorted_guild_members = guild_members.sort(
                        (a, b) => b.roles.highest.position - a.roles.highest.position,
                    );
                    sendLargeMessage(
                        message.channel.id,
                        sorted_guild_members.map((member) => `(${member.id}) ${member.user.tag}`).join('\n'),
                    );
                }
            ...
            case 'bots':
                ...
                function _output_bots() {
                    ...
                    const sorted_guild_bots = guild_bots.sort(
                        (a, b) => b.roles.highest.position - a.roles.highest.position,
                    );
                    sendLargeMessage(
                        message.channel.id,
                        sorted_guild_bots.map((member) => `(${member.id}) ${member.user.tag}`).join('\n'),
                    );
                }
            ...
            case 'usage':
                function getGuildCommandsUsage(guild_id) {
                    const current_command_logs_file = bot_command_log_file.replace(
                        '#{date}',
                        `${moment().format(`YYYY-MM`)}`,
                    );
                    const current_command_logs = require(current_command_logs_file);
                    const guild_command_usage = current_command_logs.filter(
                        (command_log_entry) => command_log_entry.guild.indexOf(guild_id) > -1,
                    ).length;
                    return guild_command_usage;
                }
                message.channel.send(
                    `Command usage for \`${guild.name}\` is \`${getGuildCommandsUsage(guild.id)}\` entered this month!`,
                );
                break;
            default:
                message.channel.send(
