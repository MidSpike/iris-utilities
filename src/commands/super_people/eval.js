...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        try {
            ...
            const code_to_run = code_input
                .trim()
                .replace(/\r?\n|\r/g, ``)
                .trim(); // removes line-breaks and trims
            const eval_output = await eval(`(async function() {${code_to_run}})();`);

            console.info(`Output:\n`, eval_output);

            const eval_output_string =
                typeof eval_output === 'string' ? eval_output : `${safe_stringify(eval_output, null, 2)}`;
