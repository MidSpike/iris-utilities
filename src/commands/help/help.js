    ...
    async executor(Discord, client, message, opts = {}) {
                    ...
                    fields: [
                        {
                            name: `Help Pages`,
                            value: `${'```'}\n${command_categories
                                .map((command_category, index) => `${index + 1} — ${command_category}`)
                                .join('\n')}\n${'```'}`,
                        },
                        {
                            name: '\u200b',
        ...
        if (specified_command_input.length === 0 || !isNaN(parseInt(specified_command_input))) {
                ...
                sendOptionsMessage(message.channel.id, makeHelpEmbed(processed_number_input), [
                    {
                        ...
                        callback(options_message) {
                            navigate_page(
                                options_message,
                                current_page_number > 1 ? current_page_number - 1 : command_categories.length,
                            );
                        },
                    },
                    {
                    ...
                    // },
                    {
                        ...
                        callback(options_message) {
                            navigate_page(
                                options_message,
                                current_page_number < command_categories.length ? current_page_number + 1 : 1,
                            );
                        },
                    },
                ]);
            ...
            if (specified_command) {
                const specified_command_aliases = specified_command.aliases.map(
                    (cmd) => `${command_prefix}${cmd.replace('#{cp}', `${command_prefix}`)}`,
                );
