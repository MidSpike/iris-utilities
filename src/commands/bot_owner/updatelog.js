    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        sendConfirmationEmbed(
            ...
            async (bot_message) => {
                ...
                const update_log_channels = client.channels.cache.filter(
                    (channel) => channel.name === bot_update_log_channel_name,
                );
                ...
                for (let update_log_channel of update_log_channels.values()) {
                    if (update_log_channel.permissionsFor(update_log_channel.guild.me).has('SEND_MESSAGES')) {
                        console.info(
                            `Sent update log to ${update_log_channel.guild.name} (${update_log_channel.guild.id})`,
                        );
                        update_log_channel.send(update_message_embed);
                    } else {
                        console.warn(
                            `Unable to send update log to ${update_log_channel.guild.name} (${update_log_channel.guild.id})`,
                        );
                    }
                    await Timer(2500);
                }
