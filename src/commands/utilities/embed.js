    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        if (message.content.replace(discord_command, ``).trim().length > 0) {
            try {
                ...
                const potential_embed_image = `${embed_segments_joined.match(regex_embed_args)?.[0]}`.replace(
                    regex_embed_args_bounds,
                    '',
                );
                const embed_image = validator.isURL(potential_embed_image) ? potential_embed_image : undefined;
                embed_segments_joined = embed_segments_joined
                    .replace(regex_embed_args, '')
                    .replace(regex_embed_args_bounds, '');
