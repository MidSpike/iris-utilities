...
async function playTTS(
    voice_channel,
    tts_text = 'Hello World! This Is The Default Text!',
    options = tts_opts_template,
) {
    ...
    const stream = `${process.env.BOT_API_SERVER_URL}/speech?token=${encodeURIComponent(
        process.env.BOT_API_SERVER_TOKEN,
    )}&type=${provider}&lang=${encodeURI(voice)}&text=${encodeURI(tts_text)}`;
    ...
    return await guild_queue_manager.addItem(
...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        const tts_arg_potential_voice = [
            ...Object.keys(google_languages_json),
            ...Object.keys(ibm_languages_json),
        ].includes(tts_arg)
            ? tts_arg
            : undefined;
        const tts_arg_potential_provider = ['ibm', 'google'].includes(tts_arg)
            ? tts_arg
            : Object.keys(google_languages_json).includes(tts_arg_potential_voice)
            ? 'google'
            : Object.keys(ibm_languages_json).includes(tts_arg_potential_voice)
            ? 'ibm'
            : undefined;

        const tts_provider =
            tts_arg_potential_provider ?? guild_config.tts_provider ?? bot_config.DEFAULT_GUILD_CONFIG.tts_provider;
        const tts_voice_when_provider_is_ibm =
            guild_config.tts_voice_ibm ?? bot_config.DEFAULT_GUILD_CONFIG.tts_voice_ibm;
        const tts_voice_when_provider_is_google =
            guild_config.tts_voice_google ?? bot_config.DEFAULT_GUILD_CONFIG.tts_voice_google;
        const tts_voice =
            tts_arg_potential_voice ??
            (tts_provider === 'ibm' ? tts_voice_when_provider_is_ibm : tts_voice_when_provider_is_google);

        const tts_source = (message_attachment
            ? (await axios.get(message_attachment.url)).data
            : (tts_input ?? '').replace(regex_tts_command_args, ``)
        ).trim();
