...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts={}) {
        ...
        const user = message.guild.members.cache.get(command_args[0]) ?? message.mentions.members.first() ?? message.member;
        if (user) {
            ...
            const member_joined_index = Array.from(message.guild.members.cache.values()).sort((a, b) => a.joinedTimestamp - b.joinedTimestamp).findIndex(m => m.id === guildMember.id);
