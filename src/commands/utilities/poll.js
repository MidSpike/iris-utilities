...
const { findCustomEmoji, constructNumberUsingEmoji } = require('../../libs/emoji.js');
            ...
            if (poll_question && poll_choices.length > 0) {
                if (poll_choices.length < 10) {
                        ...
                        new CustomRichEmbed(
                            {
                                ...
                                fields: [
                                    { name: `Poll Question`, value: `${poll_question}` },
                                    {
                                        name: `Poll Choices`,
                                        value: `${poll_choices
                                            .map((pc, i) => `${constructNumberUsingEmoji(i + 1)} — ${pc}`)
                                            .join('\n\n')}`,
                                    },
                                ],
                                footer: {
                    ...
                    for (let i = 0; i < poll_choices.length; i++) {
                        const bot_emoji = findCustomEmoji(
                            `bot_emoji_${['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine'][i]}`,
                        );
