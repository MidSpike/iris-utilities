    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        if (user_text.length === 0) {
                ...
                new CustomRichEmbed(
                    {
                        ...
                        fields: [{ name: 'Example Usage', value: `${discord_command} hallo dort aus Deutschland!` }],
                    },
                    message,
                ),
            );
            ...
            const api_response = await axios.get(
                `${bot_api_url}/translate?token=${encodeURIComponent(
                    process.env.BOT_API_SERVER_TOKEN,
                )}&translate_to=${encodeURIComponent(translate_to)}&text=${encodeURIComponent(user_text)}`,
            );
