...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        if (command_args[0]) {
            axios
                .get(`http://ip-api.com/json/${command_args[0]}?fields=query,city,regionName,country,zip,isp,org`)
                .then((res) => {
                        ...
                        new CustomRichEmbed(
                            {
                                ...
                                fields: Object.keys(res.data)
                                    ...
                                    .map((props) => ({ name: props[0], value: props[1] || 'not found' })),
                            },
                            message,
                        ),
                    );
                ...
                new CustomRichEmbed(
                    {
                        ...
                        fields: [{ name: 'Command Usage', value: `\`\`\`${discord_command} IP_ADDRESS_HERE\`\`\`` }],
                    },
                    message,
                ),
            );
