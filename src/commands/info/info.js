...
module.exports = new DisBotCommand({
    ...
    async executor(Discord, client, message, opts = {}) {
        ...
        const people_music_listeners =
            client.voice.connections
                .map((connection) => connection.channel.members.filter((member) => !member.user.bot).size)
                .reduce((a, b) => a + b, 0) ?? 0;
        const bot_music_listeners =
            client.voice.connections
                .map(
                    (connection) =>
                        connection.channel.members.filter(
            ...
            new CustomRichEmbed(
                {
                    ...
                    fields: [
                        ...
                        },
                        {
                            name: 'My Admins',
                            value: `${super_people
                                .filter((super_person) => super_person.public)
                                .map((super_person) => super_person.name)
                                .join('\n')}`,
                        },
                        {
                            name: 'My Website',
                        ...
                        },
                        {
                            ...
                            value: `${people_music_listeners} ${
                                people_music_listeners === 1 ? 'person is' : 'people are'
                            } listening to music`,
                        },
                        {
                            name: `The Number Of Bots Listening To Music`,
                            value: `${bot_music_listeners} ${
                                bot_music_listeners === 1 ? 'bot is' : 'bots are'
                            } listening to music`,
                        },
                        {
                        ...
                        },
                        {
                            ...
                            value: `${bot_special_text_channels
                                .map(
                                    (special_channel) =>
                                        `\`${special_channel.name}\` - ${
                                            client.channels.cache.filter(
                                                (channel) => channel.name === special_channel.name,
                                            ).size
                                        } Guilds`,
                                )
                                .join('\n')}`,
